...
{
    int err;
    uint8_t buf[1500]; //Receive buffer.
    uint8_t wifi_ssid[50];
    uint8_t wifi_pswd[50];
    uint8_t ack_response[] = "ACK";
    uint8_t end_response[] = "Credentials received, restarting board, please disconnect client from AP";
    uint8_t counter = 0;
    ...
    while ( 1 )
    {
        ...
        if ( bytes > 0 )
        {   //Data received, Warning the data needs to be related to WiFi credentials.
            if ( counter == 0 )
            {   //SSID received. 
                memcpy( wifi_ssid, buf, bytes );

                //Send answer to client.
                bytes = write( sck, ack_response, sizeof( ack_response ) - 1 );
                counter++;
            }

            else
            {   //PSWD received.
                memcpy( wifi_pswd, buf, bytes );
                
                //Send answer to client.
                bytes = write( sck, end_response, sizeof( end_response ) - 1 );
                CleanUpAP( ( char * ) wifi_ssid, ( char * ) wifi_pswd );    //Cleaning AP.
            }
        }
